...
| `userReadingStatus` | 예 | `reading`, `finished`, `dropped`, `ambiguous` |
| `userProgress` | 예 | `80화까지`, `완주`, `33화에서 중단`처럼 자유 표기 |
| `group` | 예 | 기존 정리 그룹. `ongoing`, `completed-section`, `ambiguous` 등 |
| `coverImage` | 예 | 표지 이미지 경로 |
| `descriptionFile` | 예 | 작품 소개 텍스트 파일 경로 |
| `userRating` | 아니오 | 개인 평점. 0부터 5까지 숫자 |
| `userReview` | 아니오 | 개인 후기 |
