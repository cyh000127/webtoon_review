...
3. `입력` 탭에서 제목, 별점, 읽은 위치, 감상 상태를 입력한다.
...
11. `최근` 탭에서 Fetch, Pull, Force 받기 버튼이 각각 오류 없이 동작하는지 확인한다.
...
- 제목이 비어 있으면 제출하지 못해야 한다.
- `읽는 중`, `중도 이탈` 상태에서 읽은 위치가 비어 있으면 제출하지 못해야 한다.
- `완주` 상태에서는 읽은 위치를 비워도 제출할 수 있어야 한다.
