...
모바일 입력 앱은 웹툰 제목, 별점, 읽은 위치, 감상 상태를 받아 대기열 파일에 커밋한다.  
...
```json
{"id":"wq_20260525_000001","title":"웹툰 제목","rating":4.5,"readProgress":"151화","readingStatus":"completed","createdAt":"2026-05-25T15:00:00.000+09:00","source":"mobile-queue-app","status":"pending"}
```
...
| `readProgress` | 조건부 | 사용자가 어디까지 봤는지 적은 문자열. 예: `151화`, `34화`. `completed`는 생략 가능 |
...
완주로 제출된 항목은 정식 아카이브 승격 시 개인 기록 기준 완결로 처리한다. 즉 `userReadingStatus`는 `finished`, `serializationStatus`는 `completed`, `group`은 `completed-section`으로 변환한다. `userProgress`에는 `완주`, `완독`, `에서 중단` 같은 상태 문구를 제거하고 `151화`처럼 화수만 남긴다.

기존 모바일 앱에서 이미 제출된 legacy 항목은 개인 리뷰 필드가 남아 있을 수 있다.  
Codex는 legacy 항목을 정리할 때 `review`, `dropReason`을 삭제하고, 필요한 경우 사용자에게 읽은 위치와 감상 상태를 확인한다.

## Processed 항목 형식

```json
{"id":"wq_20260525_000001","title":"웹툰 제목","rating":4.5,"readProgress":"151화","readingStatus":"completed","createdAt":"2026-05-25T15:00:00.000+09:00","source":"mobile-queue-app","status":"processed","processedAt":"2026-05-25T16:00:00.000+09:00","archiveId":"044","archiveTitle":"공식 웹툰 제목","commitSha":"abcdef1"}
```
...
3. 오래된 항목부터 하나씩 처리한다.
4. 공식/공개 페이지에서 공식 URL, 표지, 작가, 장르, 작품 소개, 연재 상태, 회차 수를 확인한다.
...
- 기존 항목에 별점이나 독서 상태만 보강할 수 있으면 해당 항목을 업데이트한다.
