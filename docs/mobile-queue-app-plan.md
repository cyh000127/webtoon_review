...
핸드폰에서 웹툰 제목, 개인 별점, 읽은 위치, 감상 상태만 빠르게 입력하고 GitHub 저장소의 대기열 파일에 커밋으로 남기는 모바일 입력 앱을 만든다.
...
| WTQ-006 | 제목/평점/독서 상태 제출 플로우 구현 | WTQ-005 | 대기열 append |
...
- 빈 대기열도 검증에 성공한다.
- 잘못된 평점과 빈 제목은 검증 실패한다. `reading`, `dropped` 항목은 읽은 위치가 없으면 검증 실패한다.
...
- 제목 input, 평점 선택, 독서 상태 선택, 읽은 위치 input을 만든다.
...
- 409 conflict 발생 시 최신 파일을 다시 읽고 최대 3회 재시도
